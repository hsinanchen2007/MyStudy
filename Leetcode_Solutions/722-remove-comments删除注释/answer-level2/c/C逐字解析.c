...
{
    int i, j, count, len, idx;
    bool multFlag = false;
    char** result;

    len = sizeof(char**) * sourceSize;
    ...
    for (i = 0; i < sourceSize; i++) {
        result[i] = (char*)malloc(MAX_STRING_LEN + 1);
        if (result[i] == NULL) {
            return NULL;
        }
    }
    ...
    *returnSize = 0;
    ...
    idx = 0;
    for (i = 0; i < sourceSize; i++) {
        PRINTF("\n LINE[%3d] multFlag=%d, strlen[%d]=%d", __LINE__, multFlag, i, strlen(source[i]));
        if ((strlen(source[i]) == 1) && (multFlag == false)) {
            memcpy(&(result[*returnSize][0]), &source[i][0], strlen(source[i]));
            result[*returnSize][strlen(source[i])] = '\0';
            (*returnSize)++;
            continue;
        }

        for (j = 0; j < strlen(source[i]); j++) {
            PRINTF("\n LINE[%3d] j=%d", __LINE__, j);
            if (multFlag == false) {
                // 未在多行注释场景下
                if (j < strlen(source[i]) - 1) {
                    if ((source[i][j] == '/') && (source[i][j + 1] == '/')) {
                        PRINTF(" single comment start[%d]", j);
                        if ((j > 0) && (idx != 0)) {
                        ...
                        PRINTF(" multi comment start[%d]", j);
                        multFlag = true;
                        j++;
                    } else {
                        result[*returnSize][idx] = source[i][j];
                        PRINTF(" normal text1[%d]%c", idx, source[i][j]);
                        idx++;
                    }
                } else {
                    result[*returnSize][idx] = source[i][j];
                    PRINTF(" normal text2[%d]%c", idx, source[i][j]);
                    idx++;

                    PRINTF(" normal text endline");
                    result[*returnSize][idx] = '\0';
                    idx = 0;
                    (*returnSize)++;
                }
                ...
                // 在多行注释场景下
                if ((source[i][j] == '*') && (source[i][j + 1] == '/')) {
                    PRINTF(" multi comment end[%d]", j);
                    multFlag = false;
                    j++;

                    if ((j + 1 == strlen(source[i])) && (idx != 0)) {
                        PRINTF(" normal text endline");
                        result[*returnSize][idx] = '\0';
                        idx = 0;
                        (*returnSize)++;
