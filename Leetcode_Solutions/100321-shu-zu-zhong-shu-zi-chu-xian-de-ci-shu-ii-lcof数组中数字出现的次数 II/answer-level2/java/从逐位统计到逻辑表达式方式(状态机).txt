# 前言：
本题解整理了击败100% 的答案中的解题方法，并致力于让所有人都能够看懂。为此，本题解将从统计数字出现次数的解题方式开始讲起，再推导出逐位统计的解题方式，期望以循序渐进的方式得出最终代码的思想。
...
通过对数组中各个数的二进制表示形式逐位进行观察，我们可以发现，当数组中只出现一次的那个数字(用k表示)在二进制的对应位为0时，该对应位为1在数组各个数字中出现的总次数应当为**3^n**，当k的对应位为1时，该对应位为1在数组各个数字中出现的总次数应当为**3^n + 1**，为此，我们可以统计数字中的各个位中1出现的次数，当为3^n次时，只出现一次的数字的对应位应当为0，当为3^n + 1次时，只出现一次的数字的对应位应当为1。由此，我们可以得到如下代码：
...
![公众号二维码.jpg](https://pic.leetcode-cn.com/c0f10cdbd4c5aa573e189ad291beeff48f1c6ec1ed7eca42a92762f5e40f9b39-%E5%85%AC%E4%BC%97%E5%8F%B7%E4%BA%8C%E7%BB%B4%E7%A0%81.jpg)
