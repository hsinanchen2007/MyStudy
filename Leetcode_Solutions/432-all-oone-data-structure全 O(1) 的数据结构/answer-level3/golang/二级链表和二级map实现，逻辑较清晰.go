# 思路
1. map加list可以实现O(1)的有序。
2. 本题中，需要将一个key移动到长度加一（减一）的位置，为了方便起见，按照长度不同，分成多个list，相同长度的放到一个list中。
3. 但是如果有，1:1个key,100:1个key这样的中间空缺了的情况，那么将1里面的删除后，怎么找到长度100的是个问题。所以将不同长度的链表链接起来，形成一个二级链表，这样找最大最小值就方便了。
