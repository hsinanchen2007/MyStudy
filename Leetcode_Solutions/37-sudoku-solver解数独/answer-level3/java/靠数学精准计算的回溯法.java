回溯法的尝试填写会造成大量的无用计算，我改进了回溯算法的填写过程，就是利用直观法计算唯一值。

1、当用直观法计算所有空白格子的唯一值之后没有得到完全解，这时候开启回溯算法，此时初始数组可能有一部分精确计算唯一值写入了，减少了很多回溯格子。
2、遍历空格并定义回溯格子、历史输入值，填写一个值后又再次进入直观法计算后续空格的唯一值而不是用回溯法试填。
3、每次回溯法遇到空格处无解时，回退到上一个回溯格子，并更新回溯值，如果上一个格子没有后续可选回溯值，则循环往上回退；
